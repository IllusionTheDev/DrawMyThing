...
public class ConfigurableDouble {
    ...
    public ConfigurableDouble(double val, String path, YMLBase base, int slot, Menu menu, Menu previous) {
        this.val = val;
        this.path = path;
        this.base = base;
        ...
        item = new ItemBuilder(Material.SKULL_ITEM)
                ...
                .lore("",
                        ...
                        "&cRight Click &7to decrease by 1",
                        "&cShift Right Click &7to decrease by 1",
                        "",
                        "&7Current Value: &6" + val)
                .build();

        menu.setItem(slot, item, (event) -> {
            double increment = event.isShiftClick() ? 1 : 0.1;
